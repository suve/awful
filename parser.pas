...
Type
   ...
   TConstructType = (CT_IF, CT_ELIF, CT_WHILE, CT_REPEAT);
...
Procedure Fatal(Const Ln:LongWord;Const Msg:AnsiString;Const ErrCode:LongInt = 255);
   ...
   Writeln(StdOut, '<h3>',FULLNAME,': fatal error</h3><hr>');
   ...
   DateTimeToString(DTstr, dtf_def_fpc, Now());
   Writeln(StdOut, '<p><small>Generated by ',FULLNAME,', rev.',VREVISION,', on ',DTstr,'.</small></p>');
   Writeln(StdOut, '</body>');
   Writeln(StdOut, '</html>');
   Halt(0)
   {$ELSE}
...
Procedure cstruFatal(Const Ln:LongWord);
   ...
   While (Not cstruStack^.Empty) do begin
      ...
      Case cstru.Typ of
           ...
           CT_ELIF: Writeln(StdErr,YukName,'(', IfArr[cstru.Idx][0],'): Error: !elif block stretches past ',PastWhat);
...
Function mkcstru(Const T:TConstructType; Const I:LongWord):TConstructInfo; Inline;
...
Function MakeExpr(Const ExLo,ExHi,Ln:LongInt; T:LongInt):PExpr;
   ...
   Procedure Construct_If(Const IfType:TConstructType);
      begin
      A := Length(IfArr);     // get IfArr length (we will insert at this index)
      SetLength(IfArr,(A+1)); // lengthen IfArr
      
      IfArr[A][0]:=Ln; // set !if/!elif line number
      IfArr[A][1]:=-1; // set !else expression number to "undefined"
      IfArr[A][2]:=-1; // set !fi expression number to "undefined"
      
      (* Push !if/!elif identifier on stack.                                                *
       * Later, when encountering !if/!elif, we will know which !if/!elif to match them to. *)
      cstruStack^.Push(mkcstru(IfType, A)); 
      
      // Insert one token into expression - it will contain IfArr index.
      SetLength(E^.Tok, 1);
   ...
   Procedure Construct_Else();
      ...
      If (Not (cstru.Typ In [CT_IF, CT_ELIF]))
         then Fatal(Ln,'!else inside a '+cstruType()+' block.');
      If (IfArr[cstru.Idx][1] >= 0)
         then Fatal(Ln,cstruType()+' from line '+IntToStr(IfArr[cstru.Idx][0])+' has a second !else.');
   ...
   Procedure Construct_Elif();
      begin
      If (cstruStack^.Empty) then Fatal(Ln,'!elif without corresponding !if.');
      cstru:=cstruStack^.Peek();
      
      If (Not (cstru.Typ In [CT_IF, CT_ELIF]))
         then Fatal(Ln,'!elif inside a '+cstruType()+' block.');
      If (IfArr[cstru.Idx][1] >= 0)
         then Fatal(Ln,cstruType()+' from line '+IntToStr(IfArr[cstru.Idx][0])+' has an !elif after !else.');
      
      IfArr[cstru.Idx][1]:=Pr[Proc].Num; // take the previous !if/!elif and set !else expression number
      
      // Prepare !else expression. Insert one token with IfArr entry number
      SetLength(E^.Tok, 1);
      E^.Tok[0] := NewToken(VT_INT, cstru.Idx);
      // Prepare expression arg-array to accept one entry
      SetLength(E^.Arg, 1);
      E^.Num := 0;
      // Set function pointer and refmodif info
      E^.Fun := FuncInfo_Else.Ptr;
      E^.Ref := FuncInfo_Else.Ref;
      // Add expression to function
      AddExpr(E);
      
      New(E);                // Create new expression to be returned as result
      Construct_If(CT_ELIF); // Use Construct_If to perform the !if part of !elif
      end;
   ...
   Procedure Construct_Fi();
      ...
      // In the IfArr entry, !fi expression number. (!else will jump to this position when reached from then-clause)
      IfArr[cstru.Idx][2] := Pr[Proc].Num; 
      // If !else expr num is not present, set it to !fi expr num (!if will jump if condition is not met)
      If (IfArr[cstru.Idx][1] < 0) then IfArr[cstru.Idx][1]:=Pr[Proc].Num; 
      
      (* If previous construct on stack was not an !if, it had to be !elif.              *
       * Go through constructs and set their !fi jumppoints until the parent !if is met. *)
      If (cstru.Typ <> CT_IF) then
         Repeat
            cstru := cstruStack^.Pop();
            IfArr[cstru.Idx][2] := Pr[Proc].Num; 
         Until (cstru.Typ = CT_IF);
