...
Var IfArr : Array of TIf;
    ...
    FileIncludes : Array of AnsiString;
...
Procedure Fatal(Ln:LongWord;Msg:AnsiString);
   ...
   begin
   Writeln(YukStdErr^, YukName,'(',Ln,'): Fatal: ',Msg);
   {$IFDEF CGI}
   Writeln('Content-Type: text/html; charset=UTF-8');
   Writeln(YukStdOut^);
   Writeln(YukStdOut^, '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">');
   Writeln(YukStdOut^, '<html lang="en">');
   Writeln(YukStdOut^, '<head>');
   Writeln(YukStdOut^, '<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">');
   Writeln(YukStdOut^, '<title>Error</title>');
   Writeln(YukStdOut^, '</head>');
   Writeln(YukStdOut^, '<body style="background: white; color: black">');
   Writeln(YukStdOut^, '<h3>awful-cgi: fatal error</h3><hr>');
   Writeln(YukStdOut^, '<p><strong>File:</strong> ',EncodeHTML(YukPath),'</p>');
   Writeln(YukStdOut^, '<p><strong>Line:</strong> ',Ln,'</p>');
   Writeln(YukStdOut^, '<p><i>',EncodeHTML(Msg),'</i></p>');
   DateTimeToString(DTstr, dtf_def, Now());
   Writeln(YukStdOut^, '<p><small>Generated by awful-cgi, r.',VREVISION,', on ',DTstr,'.</small></p>');
   Writeln(YukStdOut^, '</body>');
   Writeln(YukStdOut^, '</html>');
   Halt(0)
   {$ELSE}
   Halt(255)
   {$ENDIF}
...
Procedure Error(Ln:LongWord;Msg:AnsiString); Inline;
   begin Writeln(YukStdErr^,YukName,'(',Ln,'): Error: ',Msg) end;
...
Function MakeExpr(Var Tk:Array of AnsiString;Ln,T:LongInt):PExpr;
   ...
   Procedure Construct_Include(Req:Boolean);
      Var FileName,Construct:AnsiString; Constant,V:PValue; F:System.Text;
      ...
      If (Tk[T][1]='i') then FileName:=IntToStr(StrToInt(FileName)) else
      If (Tk[T][1]='h') then FileName:=HexToStr(StrToHex(FileName)) else
      If (Tk[T][1]='o') then FileName:=OctToStr(StrToOct(FileName)) else
      If (Tk[T][1]='b') then FileName:=BinToStr(StrToBin(FileName)) else
      If (Tk[T][1]='f') then FileName:=FloatToStr(StrToFloat(FileName)) else
      ...
      If (Tk[T][1]='=') then begin
         ...
         V := ValToStr(Constant); FileName := PStr(V^.Ptr)^; FreeVal(V)
         end else
      ...
      YukPath := OldPath; YukName := OldName
      end;
...
Procedure ParseFile(Var I:System.Text);
   ...
   While (Not Eof(I)) do begin
      ...
      {$IFNDEF CGI} If (Length(L)>0) then begin {$ENDIF}
         P:=Proc; E:=Pr[P].Num; ExLn:=E; //Rn := 0;
         {R:+=}ProcessLine(L, N, E);
         {If (R<>NIL) and (Length(R)>0) then begin
            If (Pr[P].Num + Length(R) > Length(Pr[P].Exp)) 
               then SetLength(Pr[P].Exp, Length(Pr[P].Exp)+32);
...
Procedure ReadFile(Var I:System.Text);
   ...
   SetLength(Pr[0].Exp, Pr[0].Num);
   If (mulico > 0) then Writeln(YukStdErr^,YukName,': multi-line comment stretches past end of code.');
   {For P:=Low(Pr) to High(Pr) do begin
       Write(StdErr,'Func #',P,' has ',Pr[P].Num,'/',Length(Pr[P].Exp),' expressions and ',
             Length(Pr[P].Arg),' arguments');
       If (Length(Pr[P].Arg)>0) then begin Write(StdErr,':');
          For A:=Low(Pr[P].Arg) to High(Pr[P].Arg)
          do Write(StdErr,' ',Pr[P].Arg[A]) end;
       Writeln(StdErr,'.')
       end;}
   If (Not IfSta^.Empty()) then begin
      A:=IfSta^.Peek(); A:=IfArr[A][0];
