...
Type
   generic GenericDynPtrTrie<Tp> = DYNPTRTRIETYPE
      Public Type
   ...
   begin
      ...
      For F:=sF to 15 do
         If (N^.Fan[F] <> NIL) then begin
            For i:=sI to 15 do
               If (N^.Fan[F]^.Nod[I] <> NIL) then
                  If (N^.Fan[F]^.Nod[I]^.Val<>NIL) then begin 
                     R:=Chr(F*16+I)+R; Exit(True)
                  end else begin
...
Procedure GenericDynPtrTrie.Purge();
   begin FreeNode(Root); Root^.Val := NIL end;

Procedure GenericDynPtrTrie.Purge(Const Proc:TDisposeProc);
