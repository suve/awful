...
Function ProcessLine(L:AnsiString;N:LongWord):Array_PExpr;
   ...
   SetLength(R, Rn); Rn := 0; P:=1;
   While (P <= High(Tk)) do begin
      If (Tk[P][1]='!') then begin //if (n=11) then writeln('makeexpr ',rs,'..',p-1);
         R[Rn] := MakeExpr(Tk[Rs..P-1], N, 0);
         If (R[Rn]<>NIL) then begin ExLn += 1; Rn += 1 end;
         Rs := P;
         end else
      If (Tk[P][1]='~') then begin
         ...
         While (P <= High(Tk)) and (Tk[P][1] = '~') do P += 1; Rs := P;
         If (P > High(Tk)) or (Tk[P][1] <> '!') then P -= 1
         end;
      P += 1
      end;
   If (Rs <= High(Tk)) and (Tk[Rs][1]<>'~') then begin
      R[Rn] := MakeExpr(Tk[Rs..High(Tk)], N, 0); 
      If (R[Rn]<>NIL) then begin ExLn += 1; Rn += 1
      end end;
...
Procedure ReadFile(I:PText);
   ...
   While (Not Eof(I^)) do begin
      ...
      {$IFNDEF CGI} If (Length(L)>0) then begin {$ENDIF}
         ...
         R:=ProcessLine(L, N);
         If (R<>NIL) and (Length(R)>0) then begin
            SetLength(Pr[P].Ex, Length(Pr[P].Ex)+Length(R));
